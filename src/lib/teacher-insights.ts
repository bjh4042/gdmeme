...
export function computeInsights(input: {
  ...
  const activeList = scope.map((s) => {
    ...
    const days = Number.isFinite(recencyMs)
      ? Math.max(0, (Date.now() - recencyMs) / 86400000)
      : 999;
  ...
  const activeStudents = activeList
    .filter((a) => a.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, 5);

  const reflectionCount = scope.filter(
    (s) => (input.byStudent[s.id]?.journals?.length ?? 0) > 0,
  ).length;
  const practiceCount = scope.filter(
    (s) => (input.byStudent[s.id]?.practiceLogs?.length ?? 0) > 0,
  ).length;
  ...
  if (scope.length === 0)
    guidance.push("아직 등록된 학생이 없습니다. 학생 명단을 업로드해 주세요.");
  else {
    if (reflectionCount / total < 0.3)
      guidance.push(
        "성찰 저널 참여가 낮아요. 다음 수업에서 '오늘 알게 된 표현'을 1분 공유해 보세요.",
      );
    if (practiceCount / total < 0.3)
      guidance.push(
        "실천 체크가 부족해요. 로드맵 카드의 '오늘의 실천 체크' 안내를 다시 짚어주세요.",
      );
    if (pendingProposals >= 5)
      guidance.push(
        `승인 대기 중인 학생 제안이 ${pendingProposals}건 있어요. 우선 확인이 필요해요.`,
      );
    if (dangerWords.length >= 3)
      guidance.push(
        "위험 등급(70점 이상) 표현이 다수 등록되었어요. 실제 사례로 함께 이야기 나눠보면 좋아요.",
      );
    if (guidance.length === 0)
      guidance.push(
        "전반적으로 참여가 안정적입니다. 이번 주는 학생들의 대체 표현을 함께 낭독해 보는 것을 추천해요.",
      );
  }
